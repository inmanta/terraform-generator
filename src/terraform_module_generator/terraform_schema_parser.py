...
def make_inmanta_safe(input: str) -> str:
    input = input.lower()

    if input in INMANTA_RESERVED_KEYWORDS:
        return input + "_x"
...
class TerraformSchemaParser:
    ...
    def parse_module(self, schema: Any):
        ...
        for resource_name, resource_schema in schema.resource_schemas.items():
            ...
            for resource_path_elem in resource_path:
                resource_path_elem = make_inmanta_safe(resource_path_elem)
    ...
    def build_resource_config_implementation(
        ...
        implementation = Implementation(
            name="config",
            path=resource_entity.path + [make_inmanta_safe(resource_raw_name)],
            entity=resource_entity,
            content=dedent(
                f"""
    ...
    def parse_entity(
        ...
        for block_type in schema_block.block_types:
            ...
            relation = EntityRelation(
                name=make_inmanta_safe(block_type.type_name),
                path=path,
                entity=entity,
                arity=(block_type.min_items, max_arity),
                peer=EntityRelation(
                    name="",
                    path=path,
        ...
        if not skip_index:
            index = Index(
                path=entity.path,
                entity=entity,
                attributes=self.base_entity.attributes[:1],
            )
    ...
    def parse_attribute(self, schema: Any) -> Attribute:
        ...
        return Attribute(
            name=make_inmanta_safe(schema.name),
            inmanta_type=self.parse_type(type),
            optional=schema.optional,
            description=schema.description,
        )

    def parse_type(self, raw_type: str) -> InmantaAttributeType:
        def parse_primitive(input_type: str) -> str:
            parsed = input_type.strip('"')
            if parsed in ["string", "bool", "number", "any"]:
                return parsed

            raise Exception(f"Unknown primitive type: {input_type}")

        def parse_aggregate(input_type: str) -> str:
            parsed = input_type.strip('"')
        ...
        if raw_type.startswith("["):
            ...
            if aggregate == "dict":
                return "dict"
            return InmantaPrimitiveList(primitive)
